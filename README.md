...
📄 **[Plano de testes](docs/02-plano-de-testes.md)** — 11 casos executados com evidência real
...
flowchart TD
    ...
    D --> E{Conferência<br/>NF-e × Pedido}
    E -->|Fornecedor divergente<br/>Preço fora da tolerância<br/>Qtde acima da aberta<br/>CFOP ou NCM incompatível| X
    ...
    style X fill:#fde8e8,stroke:#c53030,color:#1a202c
    style H fill:#e6f4ea,stroke:#2f855a,color:#1a202c
```
...
| Determinação de CFOP de entrada | J1BTAX → CFOP Determinação MM (J_1BAONV) + cadastro do material |
...
| Preço 10% acima do pedido | Variação de preço (chave PP / OMR6) | Bloqueado |
| CFOP 5551 contra pedido de insumo | Compatibilidade de natureza da operação | Bloqueado |
...
| Alíquota de 12% em operação interna | Alíquota esperada para a operação | Bloqueado |
| NCM divergente do cadastro | Dados mestres do material | Bloqueado |
...
**Camada de orquestração — n8n.** Recebe o XML, valida o documento fiscal, consulta o pedido, confronta NF-e contra pedido e decide entre lançar ou bloquear. O three-way match propriamente dito (pedido × entrada × fatura) é feito pelo sistema de registro no momento da MIRO, quando a entrada já existe. Em maio de 2026 a SAP fez investimento estratégico na n8n e firmou parceria para embarcar a plataforma no Joule Studio — este projeto usa exatamente esse padrão de orquestração sobre as APIs do S/4.
...
As validações existem **dos dois lados de propósito**: o orquestrador valida antes de chamar, e o sistema de registro valida de novo e rejeita — porque um sistema de registro nunca confia cegamente no integrador. Os casos CT-08 a CT-11 do plano de testes provam isso chamando as APIs diretamente, sem passar pelo n8n.
...
docs/
  ...
  02-plano-de-testes.md     11 casos de teste com evidência de execução
...
test-data/
  nfe_01..07.xml              NF-e de teste, uma por cenário
scripts/
...
Validação do dígito verificador de CNPJ; tratamento de NF-e com múltiplos itens e rateio; ICMS-ST e DIFAL; consulta de dados mestres no sandbox oficial do SAP Business Accelerator Hub para um cenário híbrido; classificação automática das divergências com sugestão de ação ao analista.
